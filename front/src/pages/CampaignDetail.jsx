...
export default function CampaignDetail({ wallet }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetchCampaignById(id, wallet?.address)
      .then((c) => {
  ...
  useEffect(() => {
    ...
    fetchContributions(id)
      .then((contribs) => {
        if (alive) setContributors(contribs);
      })
      .catch(() => {})
      .finally(() => {
        if (alive) setContribLoading(false);
      });
    return () => {
      alive = false;
    };
  ...
  const run = async (fn, labels, setLoader) => {
    ...
    try {
      ...
      const c = await fetchCampaignById(id, wallet?.address);
      setCampaign(c);
    } catch {
      /* toast */
    } finally {
  ...
  const remaining = Math.max(
    0,
    parseFloat(c.goalEth) - parseFloat(c.amountRaisedEth)
  );
  const parsedAmount = parseFloat(amount.replace(",", "."));
  const hasAmount = amount !== "" && !isNaN(parsedAmount) && parsedAmount > 0;
  const effectiveAmt = hasAmount
    ? Math.round(Math.min(parsedAmount, remaining) * 1e6) / 1e6
    : 0;
  const isCapped = hasAmount && parsedAmount > remaining;

  const dd = new Date(Number(c.createdAt) * 1000);
  const createdDate = `${String(dd.getDate()).padStart(2, "0")}/${String(
    dd.getMonth() + 1
  ).padStart(2, "0")}/${dd.getFullYear()}`;

  const dl = new Date(Number(c.deadline) * 1000);
  const deadlineDateTime = `${String(dl.getDate()).padStart(2, "0")}/${String(
    dl.getMonth() + 1
  ).padStart(2, "0")}/${dl.getFullYear()} ${String(dl.getHours()).padStart(
    2,
    "0"
    ...
    <div className="campaign-detail">
      ...
      <div className="fiche-hero">
        ...
        <div className="fiche-hero-panel">
          ...
          <div className="fiche-funded-row">
            <span className="fiche-funded-val">
              {c.amountRaisedEth} <span className="fiche-funded-unit">ETH</span>
            </span>
          ...
          <div className="fiche-bar-meta">
            <span
              className="fiche-pct"
              style={{
                color: c.status === "failed" ? "var(--red)" : "var(--green)",
              }}
            >
              {c.progress}% COMPLÉTÉ
            </span>
            <span className="fiche-contrib-count">
              {c.contributorCount} CONTRIBUTEURS
            </span>
          </div>
          ...
          {c.status === "active" && wallet?.connected && (
            <div className="fiche-contribute">
              <label className="fiche-contribute-label">
                MONTANT À CONTRIBUER
              </label>
              <div className="fiche-contribute-row">
                <div
                  className="input-suffix-wrap"
                ...
                <button
                  ...
                  onClick={() =>
                    run(
                      () => txContribute(c.id, effectiveAmt),
                      TX_LABELS.contribute,
                      setLoadingContrib
                    )
                  }
                >
              ...
              {isCapped && (
                <p className="fiche-cap-notice">
                  ...
                  Plafonné à <strong>{effectiveAmt} ETH</strong> — objectif
                  restant.
                </p>
              )}
            </div>
          ...
          {c.status === "active" && isCreator && c.amountRaised === 0n && (
            <div style={{ marginTop: "1rem" }}>
              <p className="fiche-cancel-notice">
                <i className="ti ti-alert-triangle" /> Aucune contribution —
                l'annulation est permanente.
              </p>
              <button
                ...
                onClick={() =>
                  run(
                    () => txCancelCampaign(c.id),
                    TX_LABELS.cancelCampaign,
                    setLoadingCancel
                  )
                }
              >
          ...
          {(c.status === "failed" || c.status === "cancelled") &&
            c.myContrib > 0n && (
              <button
                ...
                onClick={() =>
                  run(() => txRefund(c.id), TX_LABELS.refund, setLoadingRefund)
                }
              >
                {loadingRefund ? (
                  <i className="ti ti-loader-2 spinning" />
          ...
          {/* Already withdrawn (success path) */}
          {c.status === "success" && c.withdrawn && (
            <div
              className="detail-withdrawn-note"
              style={{ marginTop: "1rem" }}
            >
      ...
      <div className="fiche-info">
        ...
        <div className="fiche-meta-grid">
          <div className="fiche-meta-item">
            ...
            <span className="fiche-meta-val fiche-meta-val--green">
          ...
          <div className="fiche-meta-item">
            ...
            <span
              className={`fiche-meta-val ${
                c.status === "active" ? "fiche-meta-val--green" : ""
              }`}
            >
              {deadlineDateTime}
            </span>
        ...
        <div className="fiche-share-row">
          ...
          <span className="fiche-share-url">
            sepolia.ethfund.io/campaign/{id} · <em>{c.title}</em> · {c.progress}
            % funded
          </span>
          <button
            className="contract-copy-btn"
          ...
          >
            <i
              className={`ti ${copiedShare ? "ti-check" : "ti-copy"}`}
      ...
      {isCreator && c.status === "active" && (
        <div className="fiche-edit-panel" id="edit-panel">
          <button
            className="fiche-edit-toggle"
            onClick={() => {
              if (!editOpen) {
                setEditDesc(c.description);
                setEditPreview(ipfsUrl(c.imageIPFS));
              }
              setEditOpen((o) => !o);
            }}
          >
            ...
            <div className="fiche-edit-body">
              ...
              {c.amountRaised === 0n ? (
                  ...
                  <div className="detail-field">
                    ...
                    {editPreview ? (
                      ...
                      <label className="upload-zone">
                        ...
                        <span>
                          <strong>Choisir une image</strong> ou glisser-déposer
                        </span>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        let cid = c.imageIPFS;
                        if (editImageFile)
                          cid = await uploadToPinata(editImageFile);
                        await runTx(
                          () => txUpdateCampaignMeta(c.id, editDesc, cid),
                          {
                            pending: "UPDATING_CAMPAIGN...",
                            success: "CAMPAIGN_UPDATED",
                          }
                        );
                        const updated = await fetchCampaignById(id, wallet?.address);
                        setCampaign(updated);
                        setEditImageFile(null);
                        setEditOpen(false);
                ...
                <div className="fiche-edit-meta-locked">
                  ...
                  DESCRIPTION &amp; IMAGE verrouillés — des contributions ont
                  été reçues.
                </div>
              )}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    await runTx(() => txExtendDeadline(c.id, editExtraDays), {
                      pending: "EXTENDING_DEADLINE...",
                      success: "DEADLINE_EXTENDED",
                    });
      ...
      <div className="fiche-log">
        ...
        {contribLoading ? (
          ...
          <EmptyState
            ...
            subtitle="AUCUNE CONTRIBUTION POUR L'INSTANT."
          />
        ) : (
          [...contributors]
...
function CountdownVal({ deadline, status }) {
  ...
  if (status !== "active")
    return (
      <span className="fiche-countdown-val fiche-countdown-val--dim">
        --:--:--:--
      </span>
    );
  return (
    <span
      className={`fiche-countdown-val${
        isUrgent ? " fiche-countdown-val--urgent" : ""
      }`}
    >
      {done ? "EXPIRÉ" : display}
    </span>
